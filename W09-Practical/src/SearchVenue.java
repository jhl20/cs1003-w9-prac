...
public class SearchVenue {
    /**
     * Reads a url, constructs a xml document, processes the xml document, repeat for url nodes and prints out venue name.
    ...
    public void search(String[] args, int search, int query, Path cache) {
        try {
            ...
            t.transform(source, result);  // makes xml
             */
            Settings settings = new Settings();
            settings.buildDoc(args, search, query, cache);
            NodeList venue = settings.getDoc().getElementsByTagName("venue");
            for (int i = 0; i < venue.getLength(); i++) {
                System.out.println(venue.item(i).getTextContent());
            }
